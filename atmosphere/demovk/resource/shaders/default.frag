...
RadianceSpectrum GetSkyRadianceAtmo(
    IN(AtmosphereParameters) atmosphere,
    IN(TransmittanceTexture) transmittance_texture,
    IN(ReducedScatteringTexture) scattering_texture,
    IN(ReducedScatteringTexture) single_mie_scattering_texture,
...
#define RADIANCE_API_ENABLED
    ...
    RadianceSpectrum GetSkyRadiance(
      ...
      return GetSkyRadianceAtmo(ATMOSPHERE, transmittance_texture,
...
void main() {
  ...
  float shadow_in;
  float shadow_out;
  GetSphereShadowInOut(view_direction, sun_direction.xyz, shadow_in, shadow_out);
  float lightshaft_fadein_hack = smoothstep(
      0.02, 0.04, dot(normalize(camera.xyz - earth_center.xyz), sun_direction.xyz));
  ...
  if (distance_to_intersection > 0.0) {
    ...
    vec3 transmittance;
    vec3 in_scatter = GetSkyRadianceToPoint(camera.xyz - earth_center.xyz,
        point - earth_center.xyz, shadow_length, sun_direction.xyz, transmittance);
  ...
  if (distance_to_intersection > 0.0) {
    ...
    ground_radiance = kGroundAlbedo * (1.0 / PI) * (
        sun_irradiance * GetSunVisibility(point, sun_direction.xyz) +
  ...
  vec3 radiance = GetSkyRadiance(
      camera.xyz - earth_center.xyz, view_direction, shadow_length, sun_direction.xyz,
      transmittance);
